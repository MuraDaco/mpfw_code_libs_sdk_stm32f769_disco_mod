...
static void UART_TxISR_8BIT(USART_TypeDef *usart_instance);
static void UART_TxISR_16BIT(USART_TypeDef *usart_instance);
static void UART_EndTransmit_IT(USART_TypeDef *usart_instance);
static void UART_RxISR_8BIT(USART_TypeDef *usart_instance);
static void UART_RxISR_16BIT(USART_TypeDef *usart_instance);
/**
  * @}
  */
...
void HAL_UART_IRQHandler(UART_HandleTypeDef *huart)
{
  ...
  if (errorflags == 0U)
  {
    ...
    if(
        ((isrflags & USART_ISR_RXNE) != 0U)  && 
        ((cr1its & USART_CR1_RXNEIE) != 0U)
    ) {
      UART_RxISR_8BIT(huart->Instance);
      return;
    }
  }
  ...
  if(
      ((isrflags & USART_ISR_TXE) != 0U)  && 
      ((cr1its & USART_CR1_TXEIE) != 0U)
  ) {
    UART_TxISR_8BIT(huart->Instance);
    return;
  }

  /* UART in mode Transmitter (transmission end) -----------------------------*/
  if(
    ((isrflags & USART_ISR_TC) != 0U) && 
    ((cr1its & USART_CR1_TCIE) != 0U)
  ) {
    UART_EndTransmit_IT(huart->Instance);
    return;
  }
...
/**
  ...
  * @retval None
  */
__weak void HAL_UART_TxCpltCallback(void)
{
...
/**
  ...
  * @retval None
  */
__weak void HAL_UART_RxCpltCallback(void)
{
...
static void UART_TxISR_8BIT(USART_TypeDef *usart_instance)
{
  g_callback_protocol_functions.tx_buffer_management(usart_instance);
  if(g_callback_protocol_functions.tx_status_get__completed()) {
    /* Disable the UART Transmit Data Register Empty Interrupt */
    ATOMIC_CLEAR_BIT(usart_instance->CR1, USART_CR1_TXEIE);

    /* Enable the UART Transmit Complete Interrupt */
    ATOMIC_SET_BIT(usart_instance->CR1, USART_CR1_TCIE);
  } 
}
...
static void UART_EndTransmit_IT(USART_TypeDef *usart_instance)
{
  ...
  ATOMIC_CLEAR_BIT(usart_instance->CR1, USART_CR1_TCIE);
...
static void UART_RxISR_8BIT(USART_TypeDef *usart_instance)
{
  ...
  g_callback_protocol_functions.rx_buffer_management(usart_instance);
  if(g_callback_protocol_functions.rx_status_get__completed()) {
      /* Disable the UART Parity Error Interrupt and RXNE interrupts */
      ATOMIC_CLEAR_BIT(usart_instance->CR1, (USART_CR1_RXNEIE | USART_CR1_PEIE));

      /* Disable the UART Error Interrupt: (Frame error, noise error, overrun error) */
      ATOMIC_CLEAR_BIT(usart_instance->CR3, USART_CR3_EIE);
...
void    USART_reg_write(void* usart_instance, char p_data)	{
    ((USART_TypeDef*) usart_instance)->TDR = p_data;    
}

uint8_t USART_reg_read(void* usart_instance)	{
    return (uint16_t) READ_REG(((USART_TypeDef*) usart_instance)->RDR);
}

HAL_StatusTypeDef USART_tx_enable(USART_TypeDef *usart_instance)
{

  if(NULL == usart_instance) return HAL_ERROR;

  /* Enable the Transmit Data Register Empty interrupt */
  ATOMIC_SET_BIT(usart_instance->CR1, USART_CR1_TXEIE);

  return HAL_OK;
}


HAL_StatusTypeDef USART_rx_enable(USART_TypeDef *usart_instance)
{

  if(NULL == usart_instance) return HAL_ERROR;

  ///* Enable the UART Error Interrupt: (Frame error, noise error, overrun error) */
  //ATOMIC_SET_BIT(usart_instance->CR3, USART_CR3_EIE);

  /* Enable the Data Register Not Empty interrupt */
  ATOMIC_SET_BIT(usart_instance->CR1, USART_CR1_RXNEIE);
